...
/*
Matlab code to generate cosine
Lh = 479;
N = 100*Lh;
n = 0:1:N-1;
fo = 1/16; %fo = 1/8; %fo = 1/4;
x = cos(2*pi*fo*n);
plot(x)
fid = fopen('cos_in.txt', 'w');
fprintf(fid,'%0.10f,\r\n', x);
fclose(fid);

fid = fopen('cos_in.txt','r');
x = textscan(fid, '%f');
fclose(fid);
plot(x{1})


Matlab code to plot outputted cosine
fid = fopen('cos_16_out.bin');
